...
export const colorSpaceDisplayMode = atom<ColorSpaceDisplayModes>(ColorSpaceDisplayModes.OKLCH);

export const changeColorSpaceDisplayMode = (): void => {
	const previous = colorSpaceDisplayMode.get();

	const previousIndex = colorSpaceDisplayModesList.findIndex((mode) => mode === previous);

	const nextIndex = previousIndex >= colorSpaceDisplayModesList.length - 1 ? 0 : previousIndex + 1;
...
onMount(colorSpaceDisplayMode, () => {
	const addMessageListener = (
		...
		if (event.data?.pluginMessage.type === MessageTypes.ColorSpaceDisplayModeChange) {
			colorSpaceDisplayMode.set(event.data.pluginMessage.payload.colorSpaceDisplayMode);
		}
	};
