...
export const isP3 = computed(userSelection, (selection) => {
	return 'colorSpace' in selection ? selection.colorSpace === 'DISPLAY_P3' : false;
});

export const isMultiSelection = computed(userSelection, (selection) => {
	return 'selectedNodePairs' in selection ? selection.selectedNodePairs.length > 1 : false;
});
...
export const isInvalidBackground = computed(userSelection, (selection) => {
	return 'text' in selection && selection.text === SelectionMessageTypes.invalidBackground;
});

export const isUnprocessedBlendModes = computed(userSelection, (selection) => {
	return 'text' in selection && selection.text === SelectionMessageTypes.unprocessedBlendModes;
});
