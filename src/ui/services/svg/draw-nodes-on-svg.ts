...
import { type CanvasRect, drawFillAsRect } from '~ui/services/svg/draw-fill-as-rect.ts';
...
export const drawNodesOnSvg = (
	...
	const drawNode = (node: PolychromNode, parentGroup: SVGGElement): void => {
		...
		if (notEmpty(mappedBlendMode)) {
			svgGroup.setAttribute('style', `mix-blend-mode: ${mappedBlendMode}; isolation: isolate;`);
		}
