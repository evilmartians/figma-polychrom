...
import { apcach, type ApcachColor, apcachToCss, crTo, crToFg, maxChroma } from 'apcach';
...
const transformFgColor = (colorFg: UIColor, colorBg: UIColor): UIColor => {
	...
	const apcachLightFg = apcach(crTo(hexBg, minLc), maxChroma(oklchFg.c), oklchFg.h ?? 0);
...
const transformBgColor = (colorFg: UIColor, colorBg: UIColor): UIColor => {
	...
	const apcachDarkBg = apcach(crToFg(hexFg, minLc), maxChroma(oklchBg.c), oklchBg.h ?? 0);
...
const getThemeWithBothColorsTransformed = (colorFg: UIColor, colorBg: UIColor): Theme => {
...
const getThemeWithTransformedFg = (colorFg: UIColor, colorBg: UIColor, Lc: number): Theme => {
	const { hex: hexBg } = colorBg;

	const colorTransformedFg = transformFgColor(colorFg, colorBg);
	const themeLc = Math.round(Math.abs(Number(calcAPCA(colorTransformedFg.hex, hexBg))));
...
const getThemeWithTransformedBg = (colorFg: UIColor, colorBg: UIColor): Theme => {
	const { hex: hexFg } = colorFg;

	const colorTransformedBg = transformBgColor(colorFg, colorBg);
	const themeLc = Math.round(Math.abs(Number(calcAPCA(hexFg, colorTransformedBg.hex))));
...
const getSecondaryColor = (colorBg: UIColor): UIColor => {
	...
	if (secondaryALc >= minLc - toleranceLc && secondaryBLc >= minLc - toleranceLc) {
...
export const generateUIColors = (fg: UIColor, bg: UIColor): undefined | WidgetProps => {
