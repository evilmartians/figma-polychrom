<script lang="ts">
	...
	import { getFormatterForCSS, getFormatterForDisplaying } from '~utils/colors/formatters.ts';
	...
	const { borderColor, fill, indicatorColor, isBlended, textColor }: ColorIndicatorProps = $props();

	const formatColorForDisplay = $derived(getFormatterForDisplaying($colorSpaceDisplayMode));
