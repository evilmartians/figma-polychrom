...
declare module 'apca-w3' {
  ...
  export function calcAPCA(
    textColor: [r: number, g: number, b: number, a?: number] | number | string,
    bgColor: [r: number, g: number, b: number] | number | string,
    places?: number,
    isInt?: boolean
  ): number | string;
  ...
  export declare function reverseAPCA(
    contrast?: number,
    knownY?: number,
    knownType?: 'background' | 'bg' | 'text' | 'txt',
    returnAs?: 'color' | 'hex' | 'y' | 'Y'
  ): [number, number, number, number, string] | false | number | string;
   ...
   *
   * @param [places]
   * Number of decimal places to round the contrast to. Defaults to 2.
   *
   * @returns Array An array where the first element is the rounded contrast value
