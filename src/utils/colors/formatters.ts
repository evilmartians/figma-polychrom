...
export const clearValue = (value: number, precision = 2): number =>
	Math.round(parseFloat((value * 10 ** precision).toFixed(precision))) / 10 ** precision;

export const toPercent = (value: number): string => `${clearValue(100 * value, 0)}%`;
...
export const formatForRGBCSS = (color: Oklch, opacity?: number): string => {
	...
	const { b: b255, g: g255, r: r255 } = convertDecimalRGBto255Scale({ b, g, r });
	let postfix = '';
