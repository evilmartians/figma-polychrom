...
import { type SelectionChangeEvent, SelectionMessageTypes } from '~types/messages.ts';
...
const isValidSelection = (pair: PairState | PolychromNode): pair is PolychromNode => {
	return (
		notEmpty(pair) && pair !== PairState.InvalidBackground && pair !== PairState.InvalidBlendMode
	);
};
...
export const buildGeneralSelectionPayload = (
	...
	const selectedNodePairs = selection.filter(isValidForSelection).map((selectedNode) => {
