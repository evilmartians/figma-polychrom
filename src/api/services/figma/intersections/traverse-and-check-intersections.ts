...
const ifSelectedNodeIsChild = (node: SceneNode, selectedNode: SceneNode): boolean => {
	return 'children' in node && node.children.some((n) => n.id === selectedNode.id);
};
	...
	return nodes.reduce((accumulator: PolychromNode[], node) => {
		if (areNodesIntersecting(node, selectedNode)) {
			...
			if ('children' in node && node.children.length > 0) {
				...
				polychromNode.children = traverseAndCheckIntersections(childrenNodes, selectedNode);
			}
